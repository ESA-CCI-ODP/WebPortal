  ...
  <?php if(strpos($_SERVER['REQUEST_URI'], '/data') !== false){ ?>
	 ...
	 stopTimeout = false;
		...
		function openColorBox(){
			$.colorbox({iframe:true,  width:"525", height:"620", href: "http://cci.esa.int/sites/all/themes/drupal-contrib/ESA CCI/sign-up-form.php"})
		}
	 </script>
  <?php } ?>
  <style>
...
<?php print $closure;?>
  ...
  <?php if(strpos($_SERVER['REQUEST_URI'], '/data') !== false){ ?>
	 <script type="text/javascript">	
		if (document.cookie.indexOf("cci_portal_newsletter_signed_up=") >= 0 && readCookie("cci_portal_newsletter_signed_up") === "TRUE" ) {
		  // They've been here before.
		}
		else {
		  //createCookie("cci_portal_newsletter_signed_up","FALSE",1);
		  $.cookie('cci_portal_newsletter_signed_up', false, { expires: 1, path: '/' });
		  myFunction();
		}
	 </script>
  <?php } ?>
